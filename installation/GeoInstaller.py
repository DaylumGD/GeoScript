...
if sys.platform == 'win32':
    while True:
        ...
        if command == 1:
            ...
            shutil.copytree('libraries', data_dir)
            for file in os.listdir(main_root_dir):
                if not file in ['editing_guide.md', 'changelog.md', '.vscode', 'libraries', 'quick_fixes.md', 'GeoScript']:
                    if file == 'geoscript.exe':
                        shutil.copyfile(file, r'C:/Windows/System32/geoscript.bat')
                    if os.path.isdir(file) == True:
                        shutil.copytree(file, main_dir)
                    else:
                        shutil.copyfile(file, main_dir)
    ...
    print(f'Looks like your using {sys.platform}. As of now GeoScript is only available of Windows', file=sys.stderr)
    print('For more infomation read read below\n', file=sys.stderr)
    print("GeoScript currently only works on Windows 10 and 11, this is because as of now we have no other way of decoding the MacOS's Geometry Dash save file if you know or have an idea of how to decode it then please contact me through Discord (discord in the readme)", file=sys.stderr)
    input('\nHit enter to exit')