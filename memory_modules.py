...
def insert_backlog_memory(key, parsed_code):
    global statement_backlog
    k, v = [], []
    for ak in statement_backlog:
        k.insert(0, ak)
        v.insert(0, statement_backlog.get(ak))
    k.insert(0, key)
    v.insert(0, parsed_code)
    
    statement_backlog = {ik: iv for ik, iv in zip(k, v)}
